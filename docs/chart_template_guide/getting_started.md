...
As described in the [Charts Guide](../charts.md), Helm charts are structured like this:
...
The `Chart.yaml` file contains a description of the chart. You can access it from within a template. The `charts/` directory _may_ contain other charts \(which we call _subcharts_\). Later in this guide we will see how those work when it comes to template rendering.
